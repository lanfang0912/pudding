...
function buildShipmentEmail(rawOrder) {
  ...
  const subject = isPickup
    ? `white dessert 面交通知｜${order.id}`
    : `white dessert 出貨通知｜${order.id}`;
  ...
  const lines = [
    isPickup ? 'white dessert 面交通知' : 'white dessert 出貨通知',
    '',
    `${order.name} 您好，`,
    '',
    '您訂購的 white dessert 手工布丁',
    handoff,
    '',
    '━━━━━━━━━━━━━━━━━━━━',
    `訂單編號：${order.id}`,
...
function buildCustomerConfirmationEmail(rawOrder) {
  ...
  const subject = `white dessert 訂購確認｜${order.id}`;
  const lines = [
    `${order.name} 您好，`,
    '',
    '已收到您的訂單，謝謝你的支持。',
